...
- Treat backend/infrastructure contract packets as source of truth when provided
- If contract details are missing or conflicting, emit a narrow change request before implementation
...
When API/auth/env contract is missing → request `frontend-contract-v1` from **backend-developer** or **infra**.
